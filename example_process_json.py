...
1. Count astronauts by spacecraft and save the result.
2. Extract astronaut names and save them separately.
...
# NEW FUNCTION: To extract astronaut names from the JSON file
def extract_astronaut_names(file_path: pathlib.Path) -> list:
    """Extract a list of astronaut names from the JSON file."""
    try:
        with file_path.open('r') as file:
            astronaut_data = json.load(file)
            return [person["name"] for person in astronaut_data.get("people", [])]
    except Exception as e:
        logger.error(f"Error extracting astronaut names: {e}")
        return []
...
def process_json_file():
 ...
 # NEW FUNCTION CALL: Extract astronaut names and save them
    names_output_file = pathlib.Path(PROCESSED_DIR, "json_astronaut_names.txt")
    astronaut_names = extract_astronaut_names(input_file)

    with names_output_file.open('w') as file:
        file.write("Astronaut Names:\n")
        file.write("\n".join(astronaut_names))

    logger.info(f"Astronaut names saved to: {names_output_file}")
